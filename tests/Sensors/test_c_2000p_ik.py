...
class TestSensorC_2000P_IK:

    @allure.story("С2000Р_ИК")
    @allure.title("Позитивные сценарии: Проверка ввода значений в поле 'Название'")
    def test_c_2000p_ik_name_input_positiv(self, app, c_2000p_ik, close_modal_sensor):
    ...
    def test_c_2000p_ik_name_input_negativ(self, app, c_2000p_ik, close_modal_sensor):
        with allure.step("Проверка валидации поля"):
            app.PO_Sensors.input_name_negativ()

    @allure.story("С2000Р_ИК")
    @allure.title("Проверка названий полей")
    def test_c_2000p_titles(self, app, c_2000p_ik, close_modal_sensor):
        with allure.step("Проверка названий полей"):
            app.PO_Sensors.text_c2000p_ik_title()

    @allure.story("С2000Р_ИК")
    @allure.title("Проверка переключения чек-боксов")
    def test_c_2000p_check_box(self, app, c_2000p_ik, close_modal_sensor):
        with allure.step("Проверка включения чекбоксов"):
            app.PO_Sensors.check_box_c_2000p_on()
        with allure.step("Проверка выключения чекбоксов"):
            app.PO_Sensors.check_box_c_2000p_off()
        with allure.step("Проверка частичного выбора чекбоксов"):
            app.PO_Sensors.check_box_c_2000p_some()

    @allure.story("С2000Р_ИК")
    @allure.title("Проверка выпадающего списка: Раздел")
    def test_c_2000p_drop_list_path(self, app, c_2000p_ik, close_modal_sensor):
        with allure.step("Проверка выбора позиций из выпадающего списка"):
            app.PO_Sensors.drop_list_path_c_2000p()

    @allure.story("С2000Р_ИК")
    @allure.title("Проверка выпадающего списка: Режим индикации")
    def test_c_2000p_drop_list_indication_mode(self, app, c_2000p_ik, close_modal_sensor):
        with allure.step("Проверка выбора позиций из выпадающего списка"):
            app.PO_Sensors.drop_list_indication_mode_c_2000p()

    @allure.story("С2000Р_ИК")
    @allure.title("Проверка виджета ползунка - Чувствительность")
    def test_c_2000p_widget_slider_sensitivity(self, app, c_2000p_ik, close_modal_sensor):
        with allure.step("Проверка выбора позиций из выпадающего списка"):
            app.PO_Sensors.widget_slider_sensitivity_c_2000p()

    @allure.story("С2000Р_ИК")
    @allure.title("Проверка сохранения данных")
    def test_c_2000p_save_data(self, app, c_2000p_ik, close_modal_sensor):
        with allure.step("Генерирование тестовых данных"):
            app.ganerate_data.createData()
        with allure.step("Заполнение полей данными"):
            app.PO_Sensors.save_data_c_2000p_ik()
        ...
        with allure.step("Проверка сохранения"):
            app.PO_Sensors.assert_save_data_c_2000p_ik()
