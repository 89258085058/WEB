...
class TestSensorC_2000P_IRP:

    @allure.story("С2000Р_ИРП")
    @allure.title("Позитивные сценарии: Проверка ввода значений в поле 'Название'")
    def test_c_2000p_irp_name_input_positiv(self, app, c_2000p_irp, close_modal_sensor):
    ...
    def test_c_2000p_irp_name_input_negativ(self, app, c_2000p_irp, close_modal_sensor):
        with allure.step("Проверка валидации поля"):
            app.PO_Sensors.input_name_negativ()

    @allure.story("С2000Р_ИРП")
    @allure.title("Проверка названий полей")
    def test_c_2000p_irp_titles(self, app, c_2000p_irp, close_modal_sensor):
        with allure.step("Проверка названий полей"):
            app.PO_Sensors.text_c2000p_irp_title()

    @allure.story("С2000Р_ИРП")
    @allure.title("Проверка переключения чек-боксов")
    def test_c_2000p_irp_check_box(self, app, c_2000p_irp, close_modal_sensor):
    ...
    def test_c_2000p_irp_drop_list_path(self, app, c_2000p_irp, close_modal_sensor):
    ...
    def test_c_2000p_irp_drop_list_indication_mode(self, app, c_2000p_irp, close_modal_sensor):
        with allure.step("Проверка выбора позиций из выпадающего списка"):
            app.PO_Sensors.drop_list_indication_mode_c_2000p_irp()

    @allure.story("С2000Р_ИРП")
    @allure.title("Проверка сохранения данных")
    def test_c_2000p_irp_save_data(self, app, c_2000p_irp, close_modal_sensor):
