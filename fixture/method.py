...
class MethodsHelper:
    ...
    def assert_element_text(self, locator, text):
        try:
            wd = self.app.wd
            element = WebDriverWait(wd, 5).until(EC.presence_of_element_located((By.XPATH, locator)))
            element = element.text
            assert element == text, f"\nОжидаемый текст  : {text}" \
                                    f"\nФактический текст: {element}"
        except Exception as e:
            assert e == TimeoutException, f"Ошибка, локатор {locator} - не найден"

    def element_wait_to_be_text(self, locator, text):
        wd = self.app.wd
        try:
            WebDriverWait(wd, 5).until(EC.text_to_be_present_in_element((By.CSS_SELECTOR, locator), text))
        except Exception as e:
            assert e == TimeoutException, f"Искомый текст '{text}' не отобразился"

    def assert_count_elements(self, field_locator, expected_result):
        try:
            wd = self.app.wd
            if expected_result > 0:
                WebDriverWait(wd, 5).until(
                    EC.presence_of_element_located((By.XPATH, field_locator)))
            actual_result = len(wd.find_elements(By.XPATH, field_locator))
            assert actual_result == expected_result, \
                f"\nОжидаемый результат = {expected_result},\nФактический = {actual_result}"
        except Exception as e:
            assert e == TimeoutException, f"Ошибка, локатор {field_locator} - не найден"

    def go_to_element(self, locator, value):
        try:
            wd = self.app.wd
            element = WebDriverWait(wd, 5).until(
                EC.presence_of_element_located((By.XPATH, locator)))
            wd.execute_script("arguments[0].scrollIntoView(%s);" % value, element)
            time.sleep(0.3)
        except Exception as e:
            assert e == TimeoutException, f"Ошибка, локатор {locator} - не найден"

    def check_hide_element(self, locator_element, locator_action):
        try:
            wd = self.app.wd
            element = WebDriverWait(wd, 5).until(
                EC.visibility_of_element_located((By.CSS_SELECTOR, locator_element)))
            self.click_element_locate(locator_action)
            WebDriverWait(wd, 13).until(EC.staleness_of(element))
        except Exception as e:
